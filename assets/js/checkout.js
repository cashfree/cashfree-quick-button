/* global cb_cashfree_checkout_params */
jQuery( document ).ready(
	function( $ ) {
		let dismissflag = true;
		//Set env
		const  env  =  cb_cashfree_checkout_params.environment; //or production
		//Create Success Callback
		const  successCallback  =  function (data) {
			dismissflag = false;
			woocommerceFormSubmit(data, cb_cashfree_checkout_params.capture_url);
		}
		//Create Failure Callback
		const  failureCallback  =  function (data) {
			woocommerceFormSubmit(data, cb_cashfree_checkout_params.cancel_url);
		}
		...
		const  dismissCallback  =  function () {
			if(dismissflag) {
				...
				pippin_form.action=cb_cashfree_checkout_params.dismiss_url;
		...
		Pippin(env, cb_cashfree_checkout_params.token, successCallback, failureCallback, dismissCallback);
	}
);
